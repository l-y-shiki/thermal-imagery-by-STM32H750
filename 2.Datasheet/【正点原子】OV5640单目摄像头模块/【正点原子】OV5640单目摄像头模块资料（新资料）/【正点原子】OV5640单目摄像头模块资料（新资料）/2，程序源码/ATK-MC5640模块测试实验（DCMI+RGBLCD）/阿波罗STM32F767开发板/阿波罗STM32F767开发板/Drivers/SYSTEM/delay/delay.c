...
/**
 * @brief     初始化延迟函数
 * @param     sysclk: 系统时钟频率, 即CPU频率(rcc_c_ck), 216Mhz
 * @retval    无
 */  
 ...
 * @brief     延时nus
 * @param     nus: 要延时的us数
 * @note      nus取值范围: 0~19884108(最大值即2^32 / g_fac_us @g_fac_us = 216)
 * @retval    无
 */ 
 ...
 * @brief       延时nus
 * @param       nus: 要延时的us数.
 * @note        注意: nus的值,不要大于77672us(最大值即2^24 / g_fac_us @g_fac_us = 216)
 * @retval      无
 */
void delay_us(uint32_t nus)
{
...
void delay_ms(uint16_t nms)
{   
    uint32_t repeat = nms / 65;     /*  这里用65,是考虑到可能有超频应用,
                                     *  比如248Mhz的时候, delay_us最大只能延时67650us左右了
                                     */
    uint32_t remain = nms % 65;

    while (repeat)
    {
        delay_us(65 * 1000);        /* 利用delay_us 实现 1000ms 延时 */
        repeat--;
    }
