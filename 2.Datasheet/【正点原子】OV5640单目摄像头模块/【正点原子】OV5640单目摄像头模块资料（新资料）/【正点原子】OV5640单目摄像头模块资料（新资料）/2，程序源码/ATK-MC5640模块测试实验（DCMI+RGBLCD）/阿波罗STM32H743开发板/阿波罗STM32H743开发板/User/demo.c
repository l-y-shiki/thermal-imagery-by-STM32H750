 ...
 * @version     V1.0
 * @date        2022-06-21
 * @brief       ATK-MC5640模块测试实验（DCMI+RGBLCD）
...
uint16_t framebuffer[OUTPUT_HEIGHT*OUTPUT_WIDTH] __attribute__((at(0xC0000000 + 2 * (1280 * 800 * 2))));
...
void demo_run(void)
{
    ...
    {
        /* 获取图像数据 */
        atk_mc5640_get_frame((uint32_t)&framebuffer, ATK_MC5640_GET_TYPE_DTS_16B_INC, NULL);
        
        /* 显示至LCD */
        for (height_index=0; height_index<OUTPUT_HEIGHT; height_index++)
        {
            for (width_index=0; width_index<OUTPUT_WIDTH; width_index++)
            {
                lcd_draw_point(width_index, height_index, framebuffer[height_index * OUTPUT_WIDTH + width_index]);
            }
        }
    }
}
