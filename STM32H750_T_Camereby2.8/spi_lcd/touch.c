...
{ 	 
	...
	Num>>=4;   	//只有高12位有效.
	TCS1();		//释放片选	 
		m=HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_5);
	printf("cshi:%d\n",m);
	return(Num);  
//#endif
}
...
//  cs  pc5
	...
	GPIO_Initure.Mode=GPIO_MODE_INPUT;  //输入上拉
		GPIO_Initure.Pull=GPIO_PULLUP;          //上拉
//		GPIO_Initure.Speed=GPIO_SPEED_FREQ_HIGH;//高速
	HAL_GPIO_Init(GPIOB,&GPIO_Initure);	 
  	...
  	TP_Read_XY(&tp_dev.x,&tp_dev.y);//第一次读取初始化	 
		if(TP_Read_XY(&tp_dev.x,&tp_dev.y)==1)
		{
			printf("read ok!!!!!!!\n");
		}
		printf("x:%d       y:%d\n",tp_dev.x,tp_dev.y);
	...
	if(TP_Get_Adjdata()){
		printf("24xxchushihua ok\n");
		return 0;//已经校准
	}
	else			   //未校准?
	{ 			
printf("24xxchushihua no\n");		
		LCD_Clear(WHITE);//清屏
	    TP_Adjust();  //屏幕校准 
		TP_Save_Adjdata();	 
	}			
