...
/**
  * @brief  生成字符串的像素颜色数组（适配二维字库）
  * @param  str    : 输入的字符串（纯ASCII字符）
  * @param  fc     : 前景色
  * @param  bc     : 背景色
...
//			printf(":%d\n",char_code);
        ...
        for (uint8_t row = 0; row < 16; row++) {
            uint8_t line = font_data[row]; // 当前行的像素数据
//            printf(":%d\n",font_data[row]);
            // 遍历当前行的每一列（共8列，从左到右）
            for (uint8_t col = 0; col < 8; col++) {
                // 判断当前位是否为1（最高位对应最左侧像素）
                if (line & (0x80 >> col)) { // 0x80 >> col 表示从左到右扫描位
                    buf[32*row+8-col+8*c] = fc;   // 前景色
                } else {
                    buf[32*row+8-col+8*c] = bc;   // 背景色
                }
            }
        }
    }
